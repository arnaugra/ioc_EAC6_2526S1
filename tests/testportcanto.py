...
class TestGenerarDataset(unittest.TestCase):
	...
	def test_valorsmitjatp(self):
		...
		arr_tp = [row["tp"] for row in arr] # la columna tp és la segona
	...
	def test_valorsmitjatb(self):
		...
		"""
		arr = generar_dataset(100, 1, dicc)
		arr_tb = [row["tb"] for row in arr] # la columna tp és la segona
		tb_mig = sum(arr_tb)/len(arr_tb)
		self.assertGreater(tb_mig, 1600)
...
class TestClustersCiclistes(unittest.TestCase):
	...
	ciclistes_data_clean = ciclistes_data_clean.drop('name', axis=1) # eliminem el tipus, ja no interessa
